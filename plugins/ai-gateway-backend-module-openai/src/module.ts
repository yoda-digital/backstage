...
/** Registers the OpenAI AI provider with the AI Gateway. */
export const aiGatewayModuleOpenAi = createBackendModule({
  pluginId: 'ai-gateway',
  moduleId: 'openai',
  register(reg) {
    reg.registerInit({
      ...
      async init({ config, logger, providers }) {
        ...
        if (!providerConfig) {
          logger.warn(
            'OpenAI AI provider is not configured, skipping registration',
          );
          return;
        }

        const apiKey = providerConfig.getString('apiKey');
        const organization = providerConfig.getOptionalString('organization');
        const baseUrl = providerConfig.getOptionalString('baseUrl');

        providers.registerProvider(
          OpenAiProvider.create({ apiKey, organization, baseUrl }, logger),
        );
      },
    });
  },
